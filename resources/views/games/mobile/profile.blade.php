...
<div class="row mb-3">
    <div class="col-6 text-center">
        <img src="{{ $profile->photo }}" class="rounded-circle profileAvatar">
    </div>
    <div class="col-6">
        <div class="row mb-3">
            <div class="col">
                <h2>
                    {{ $profile->nicknameWithPhone() }}
                </h2>
                <p style="color: slategray">{{ $profile->note }}</p>
            </div>
            
        </div>
        <div class="row">
            
            <div class="col">有效積分：{{ $profile->gameIntegral(52) }}</div>
            <!-- <div class="col-md-9">{{ $profile->gameIntegral(52) }}</div> -->
        </div>
        <div class="row">
            <div class="col">累計積分：{{ $profile->gameIntegral() }}</div>
            <!-- <div class="col-md-9">{{ $profile->gameIntegral() }}</div> -->
        </div>
        <div class="row">
            <div class="col">斬蝦數：{{ $profile->gamePoint() }}</div>
            ...
            <tbody>
                ...
                <tr>
                    <td class="text-center">
                        <a href="{{ url('api/game') }}/{{ $record->game->identifier }}?api_token={{ $api_token }}&imei={{ $imei }}&user_id={{ $record->user_id }}">{{ $record->game->identifier }}</a>
                    </td>
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-hover">
            ...
            <tbody>
                ...
                <tr class='clickable-row' data-href="{{ url('api/game') }}/{{ $record->game->identifier }}?api_token={{ $api_token }}&imei={{ $imei }}&user_id={{ $record->user_id }}">
                    <td class="text-center">
                       {{ $record->game->identifier }}
                    </td>
                    <td class="text-left">{{ $record->game->shrimpFarm->name }}</td>
                    <td class="text-center">{{ $record->result ?? '未得名' }}</td>
                    <td class="text-center">{{ $record->member()->results()->sum('point') }}</td>
                    <td class="text-center">{{ $record->integral }}</td>
                </tr>
                @empty
                <tr>
