...
<div class="row">
    <div class="col-md-12 my-3">
        ...
        <a href="{{ url('game') }}/create" class="btn btn-primary float-right" role="button" aria-pressed="true">新增比賽</a>
    </div>
</div>
      ...
      <tbody v-if="records.length">
        <tr v-for="(record,index) in records" v-bind:data-id="record.id">
          ...
          <td class="text-center" width="10%">
            <a :href="`{{ url('game') }}/${record.id}/edit`" v-if="record.status != 'cancel'" class="btn btn-primary" role="button" aria-pressed="true">修改</a>
            <a :href="`{{ url('game') }}/${record.id}/pushPending`" v-if="record.status != 'cancel'" class="btn btn-primary" role="button" aria-pressed="true">催繳</a>
          </td>
        </tr>
      </tbody>
      <tbody v-else>
